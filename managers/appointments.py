...
class AppointmentsUserManager:
    ...
    def create_appointment(data, user):
        ...
        data["date_of_appointment"] = datetime.strptime(
            data["date_of_appointment"], "%Y-%m-%d"
        ).date()
        data["patient_id"] = user.id

        data["doctor_id"] = (
            DoctorModel.query.filter_by(
        ...
        for available_time in AppointmentsModel.query.filter_by(
            doctor_id=data["doctor_id"]
        ).all():
            if (
                data["date_of_appointment"] == available_time.date_of_appointment
                and data["hour_of_appointment"] == available_time.hour_of_appointment
            ):
                raise BadRequest(
                    "The doctor is not available! Please try another appointment!"
                )
            elif data["date_of_appointment"] < today:
                raise BadRequest("Such date is in the past!")
            elif (
                data["hour_of_appointment"] <= current_time
                and data["date_of_appointment"] == today
            ):
...
class AppointmentsPostActionManager:
    ...
    def get_needed_data_for_acton(appointment_id):
        ...
        all_patient_info = PatientModel.query.filter_by(
            id=all_app_info.patient_id
        ).first()
    ...
    def approve(appointment_id, user_id):
        ...
        Args:
            appointment_id: primary key of the appointment
            user_id: primary key of patient
        """
        (
            first_name,
            email,
        ...
        if user_id == doctor_id:
            AppointmentsModel.query.filter_by(id=appointment_id).update(
                {"status": AppointmentStatus.approved}
            )
            aws_ses.send_email(
                email=email,
                email_body=approved_appointment(
                    first_name=first_name, date_of_appointment=date_of_appointment
                ),
            )
        else:
    ...
    def reject(appointment_id, user_id):
        ...
        if user_id == doctor_id:
            ...
            aws_ses.send_email(
                email=email,
                email_body=rejected_appointment(
                    first_name=first_name, date_of_appointment=date_of_appointment
                ),
            )
        else:
